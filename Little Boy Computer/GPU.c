...
void draw_background()
{
    int gridSize = 64;
    float size = 2.0f / gridSize;
    uint16_t tile_counter = 0x0600;
    uint16_t rom_address = 0x0000;
    uint8_t colour = 0x00;

    while (tile_counter < 0x0680)
    {
        rom_address = 0x0000;
        ...
        for (int i = 0; i < 8; i++)
        {
            for (int j = 0; j < 8; j++)
            {
                int tile_count = (tile_counter - 0x0602)/2; //+2 because the counter already incremented
...
void GPUtick() {
    ...
    {
        setValue(0x0600 + i, 0x8F);
        setValue(0x0601 + i, 0xFF);
    }
    /*END TEST*/
    draw_background();
